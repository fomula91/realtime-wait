...
운영자 두 명이 같은 대기자를 동시에 호출하면 상태가 꼬일 수 있는 **동시성 문제**를 직접 코드에서 발견했습니다. 상태를 바꾸는 쿼리에 "지금 상태가 맞을 때만 바꾼다"는 조건이 빠져 있었던 것입니다. 그래서 이미 처리된 요청에서 나오는 `409` 응답을 *에러가 아니라 정상 신호*로 재정의하고, 실제 운영 확장 시 데이터베이스가 충돌을 막도록 하는 방어책(조건부 UPDATE)을 설계했습니다.
